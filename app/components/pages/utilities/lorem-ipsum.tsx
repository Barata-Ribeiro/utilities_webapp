            ...
            case 'words': {
                ...
                if (amount <= pool.length) {
                    ...
                    for (let i = poolCopy.length - 1; i > 0; i--) {
                        ...
                        const currentWord = poolCopy[i];
                        const randomWord = poolCopy[j];

                        if (!currentWord || !randomWord) {
                            throw new Error('Unexpected undefined value in poolCopy during shuffling.');
                        }

                        poolCopy[i] = randomWord;
                        poolCopy[j] = currentWord;
                    }
                    for (let w = 0; w < amount; w++) {
                        const currentWord = poolCopy[w];

                        if (!currentWord) {
                            throw new Error('Unexpected undefined value in poolCopy during word selection.');
                        }

                        words.push(capitalizeFirstLetter(currentWord));
                    }
                } else {
                    for (let w = 0; w < amount; w++) {
                        const idx = floor(random() * pool.length);
                        const selectedWord = pool[idx];

                        if (!selectedWord) {
                            throw new Error('Unexpected undefined value in pool during random selection.');
                        }

                        words.push(capitalizeFirstLetter(selectedWord));
                    }
                }
