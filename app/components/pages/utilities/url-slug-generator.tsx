...
function cleanLocale(locales: Record<string, Record<string, string>>) {
    return Object.keys(locales).reduce(
        (all, locale) => {
            ...
            all[locale] = Object.keys(localeMap)
...
export default function UrlSlugGenerator() {
    ...
    const onSubmit = useCallback(
        async (data: SlugifyType) => {
            ...
            const charMapSource = savedCharMap.current;
            const localeSource = savedLocale.current;

            if (!charMapSource || !localeSource) {
                return;
            }

            const charMap = sort(charMapSource);
            const locales = cleanLocale(localeSource);
    ...
    useEffect(() => {
        ...
        setBrowserLocale(nav.toLowerCase().split('-')[0] ?? 'en');
    }, []);
