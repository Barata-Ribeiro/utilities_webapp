...
export default function Bmi() {
    const [bmi, setBmi] = useState<BmiResult>({ result: null, height: null, weight: null });
    ...
    function onFormSubmit(data: BmiSchemaType) {
        ...
        setBmi({ result: Number.parseFloat(calculatedBmi.toFixed(2)), height: data.height, weight: data.weight });
    }
            ...
            <form onSubmit={form.handleSubmit(onFormSubmit)} className="grid gap-4">
                ...
                <div className="mx-auto inline-flex items-center gap-x-2">
                    ...
                    <Button
                        ...
                        onClick={() => {
                            form.reset();
                            setBmi({ result: null, height: null, weight: null });
                        }}
                    >
                        Reset
                    </Button>
            ...
            {!!bmi.result && (
                <Card className="mx-auto mt-4 w-full max-w-md" aria-labelledby="bmi-card-title">
                    ...
                    <CardContent className="grid space-y-6">
                        ...
                        <Badge
                            variant="secondary"
                            className={twMerge('mx-auto select-none', badgeStyle[getBmiCategory(bmi.result)])}
                        >
                            {getBmiCategory(bmi.result) === 'Normal weight' ? (
                                <BadgeCheckIcon aria-hidden />
                            ) : (
                                <BadgeAlertIcon aria-hidden />
                            )}
                            {getBmiCategory(bmi.result)}
                        </Badge>

                        <div className="grid gap-1.5">
                            <Progress
                                value={getProgressBarValue(bmi.result)}
                                aria-label="BMI progress"
                                aria-valuemin={0}
                                aria-valuemax={100}
                                aria-valuenow={getProgressBarValue(bmi.result)}
                            />
